...
- (void) addBgView {
    ...
    //起始点
    layer.startPoint = CGPointMake(1, 0.2);
    //结束点
    layer.endPoint = CGPointMake(1, 0.8);
...
- (void)initialize {
    ...
    //设置选中的小圆点的颜色--绿色
    _selectedDotColor = [UIColor colorWithRed:((float)((0x0BBF06 & 0xFF0000) >> 16))/255.0 green:((float)((0x0BBF06 & 0xFF00) >> 8))/255.0 blue:((float)(0x0BBF06 & 0xFF))/255.0 alpha:1.0];
    _dotSize = 10;
...
- (void)updatePageControl {
    ...
    if (self.pageControlAlignmentH == BHInfiniteScrollViewPageControlAlignHorizontalRight) {
        /*当前的APP需要距离右边有一点间隙，所以减掉22的距离*/
        x = CGRectGetWidth(self.frame) - size.width - self.pageControlAlignmentOffset.width-22;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row > 0) {
//NSLog(@"BHInfiniteScrollViewCell----%ld",indexPath.row % 6);
        long currentIndex = indexPath.row % self.titlesArray.count;
        NSString *currentTitle = self.titlesArray[currentIndex];
        cell.titleView.titleText = currentTitle;
    }
    
    if (self.imagesArray.count) {
        NSInteger index = indexPath.row % self.imagesArray.count;
        cell.attributes = self.tags[index];
