...
#define MessageMin_H    60.0f        // messageLbale的最小高度
#define MessageMax_H    120.0f       // messageLabel的最大高度，当超过时，文本会以...结尾
#define QLTitle_H       20.0f
#define QLBtn_H         30.0f
...
#define QLTitleFont         [UIFont boldSystemFontOfSize:17];
#define QLMessageFont       [UIFont systemFontOfSize:14];
#define QLBtnTitleFont      [UIFont systemFontOfSize:15];
...
-(instancetype)initWithTitle:(NSString *)title message:(NSString *)message cancelBtnTitle:(NSString *)cancelTitle otherBtnTitle:(NSString *)otherBtnTitle clickIndexBlock:(QLAlertClickIndexBlock)block {
    if(self=[super init]){
        ...
        if (title) {
            _titleLab=[[UILabel alloc] initWithFrame:CGRectMake(0, 10, AlertView_W, QLTitle_H)];
            _titleLab.text=title;
            _titleLab.textAlignment=NSTextAlignmentCenter;
            _titleLab.textColor=[UIColor blackColor];
            _titleLab.font=QLTitleFont;
        ...
        _messageLab.font=QLMessageFont;
        ...
        if (!cancelTitle && !otherBtnTitle) {
            _alertView.frame=CGRectMake(0, 0, AlertView_W, _messageLab.frame.size.height+QLTitle_H+40);
        } else {
            _alertView.frame=CGRectMake(0, 0, AlertView_W, _messageLab.frame.size.height+QLTitle_H+QLBtn_H+40);
        }
        ...
        if (cancelTitle) {
            ...
            _cancelBtn.titleLabel.font=QLBtnTitleFont;
        ...
        if (otherBtnTitle) {
            ...
            _otherBtn.titleLabel.font=QLBtnTitleFont;
        ...
        if (cancelTitle && !otherBtnTitle) {
            _cancelBtn.tag=0;
            _cancelBtn.frame=CGRectMake(btnLeftSpace, btn_y, AlertView_W-btnLeftSpace*2, QLBtn_H);
        }else if (!cancelTitle && otherBtnTitle){
            _otherBtn.tag=0;
            _otherBtn.frame=CGRectMake(btnLeftSpace, btn_y, AlertView_W-btnLeftSpace*2, QLBtn_H);
        }else if (cancelTitle && otherBtnTitle){
            _cancelBtn.tag=0;
            _otherBtn.tag=1;
            CGFloat btnSpace = 20;//两个btn之间的间距
            CGFloat btn_w =(AlertView_W-btnLeftSpace*2-btnSpace)/2;
            _cancelBtn.frame=CGRectMake(btnLeftSpace, btn_y, btn_w, QLBtn_H);
            _otherBtn.frame=CGRectMake(_alertView.frame.size.width-btn_w-btnLeftSpace, btn_y, btn_w, QLBtn_H);
        }
...
-(void)setAnimationStyle:(QLShowAnimationStyle)animationStyle{
