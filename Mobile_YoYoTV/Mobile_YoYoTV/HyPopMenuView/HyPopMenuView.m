...
- (CGRect)getFrameAtIndex:(NSUInteger)index;
{
    NSInteger column = 2;
    CGFloat margin = 50;
    CGFloat buttonWidth = (ScreenWidth - (column + 1)*margin) / column;
    CGFloat buttonHeight = buttonWidth - 10;
    NSInteger colnumIndex = index % column; // 列
    NSInteger rowIndex = index / column; // 行
    NSInteger totalHeight = (_dataSource.count / column) * buttonHeight;

    CGFloat x = (colnumIndex + 1) * margin + colnumIndex * buttonWidth;
    CGFloat y = ScreenHeight - 52 - 40 - totalHeight + rowIndex *buttonHeight;
...
//    NSInteger column = 2;
//    CGFloat margin = 20;
//    CGFloat buttonViewWidth = (MIN(CGRectGetWidth(self.frame), CGRectGetHeight(self.frame)) - (column + 1)*margin) / column;
