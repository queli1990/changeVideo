...
- (void)creatVideoCachesTable
{
    ...
    [_dbQueue inDatabase:^(FMDatabase *db) {
        // 创表
        BOOL result = [db executeUpdate:@"CREATE TABLE IF NOT EXISTS t_videoCaches (id integer PRIMARY KEY AUTOINCREMENT, vid text, fileName text, url text, resumeData blob, totalFileSize integer, tmpFileSize integer, state integer, progress float, lastSpeedTime integer, intervalFileSize integer, lastStateTime integer, imageURL text)"];
        if (result) {
//            HWLog(@"视频缓存数据表创建成功");
        }else {
            HWLog(@"视频缓存数据表创建失败");
...
{
    [_dbQueue inDatabase:^(FMDatabase *db) {
        BOOL result = [db executeUpdate:@"INSERT INTO t_videoCaches (vid, fileName, url, resumeData, totalFileSize, tmpFileSize, state, progress, lastSpeedTime, intervalFileSize, lastStateTime, imageURL) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", model.vid, model.fileName, model.url, model.resumeData, [NSNumber numberWithInteger:model.totalFileSize], [NSNumber numberWithInteger:model.tmpFileSize], [NSNumber numberWithInteger:model.state], [NSNumber numberWithFloat:model.progress], [NSNumber numberWithInteger:0], [NSNumber numberWithInteger:0], [NSNumber numberWithInteger:0], model.imageURL];
...
- (void)postStateChangeNotificationWithFMDatabase:(FMDatabase *)db model:(HWDownloadModel *)model
{
    ...
    if (oldState != model.state && oldState != HWDownloadStateFinish) {
        ...
        if (model.state == HWDownloadStateFinish) {
            ...
            NSDictionary *albumEpisodes = @{@"episode":strArray[1],
                                            @"success":@1};
            NSArray *nameArray = [model.fileName componentsSeparatedByString:@" "];
            NSArray *tempArray = [NSArray arrayWithObject:albumEpisodes];
            NSDictionary *params = @{@"deviceId":[YDDevice getUQID],
                                     @"albumId":strArray[0],
                                     @"albumName":nameArray[0],
